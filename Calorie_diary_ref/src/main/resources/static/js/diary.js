...
function showStories() {
  ...
  storiesObj.forEach(function (element, index) {
    html += `
              <div class="noteCard my-2 mx-2 card" style="width: 18rem;">
                      <div class="card-body">
                          <h5 class="card-title">다이어리 ${index + 1}</h5>
                          <p class="card-text"> ${element}</p>
                          <button id="${index}"onclick="deleteStory(this.id)" class="btn btn-outline-success">다이어리 삭제</button>
                      </div>
                  </div>`;
  });
  ...
  if (storiesObj.length != 0) {
    ...
    storiesElm.innerHTML = `새 다이어리를 등록해주세요`;
  }
}
